...
class CategoryService {
    ...
    async _delete(identifier: string, expectedVersion?: number) {
        const targetUUID = this.resolveCategoryUUID(identifier);
        if (!targetUUID) {
            throw new Error('Geçerli bir kategori kimliği bulunamadı.');
        }
        ...
        try {
            ...
            // Version mismatch durumunda güncel versiyonu alıp tekrar dene
    ...
    async hardDelete(identifier: string) {
        const targetUUID = this.resolveCategoryUUID(identifier);
        if (!targetUUID) {
            throw new Error('Geçerli bir kategori kimliği bulunamadı.');
        }

        await axiosInstance.delete(`${baseUrl}/${targetUUID}`);
    }
    ...
    async listProperties(categoryId: string, _categoryName?: string, _categorySlug?: string): Promise<CategoryProperty[]> {
        const targetUUID = this.resolveCategoryUUID(categoryId);
        if (!targetUUID) {
            return [];
        }
        ...
        if (response.data?.items && Array.isArray(response.data.items)) {
            return [...response.data.items].sort((a, b) => (a.sortOrder || 1) - (b.sortOrder || 1));
        }
    ...
    async syncDefaultTemplateProperties(
        ...
        return this.listProperties(categoryId);
    }
    ...
    async setPropertyActive(
        ...
        if (!isUUID) {
            throw new Error('Geçerli bir özellik kimliği bulunamadı.');
        }
        ...
        try {
            ...
            if (freshProp) {
                ...
                return retryRes.data as CategoryProperty;
            }
            throw new Error('Özellik bulunamadı.');
        }
    }

    async deleteProperty(categoryId: string, propertyId: string, _version?: number) {
        const targetUUID = this.resolveCategoryUUID(categoryId) || categoryId;
        const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(propertyId);
        const isCatUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(targetUUID);
        if (!isUUID || !isCatUUID) {
            throw new Error('Geçerli bir kimlik bulunamadı.');
        }

        try {
            await axiosInstance.delete(`${baseUrl}/${targetUUID}/properties/${propertyId}`);
        } catch {
            // Silme desteklenmiyorsa pasife al
            await this.setPropertyActive(targetUUID, propertyId, _version ?? 1, false);
        }
    }
